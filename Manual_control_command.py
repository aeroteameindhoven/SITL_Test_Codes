...
class GpsFollower(Node):
    def __init__(self):
        ...
        self.altitude_rate_pid = PID(kp=1.0, ki=0.0, kd=0.3, integral_limit=0.0)
        self.lateral_rate_pid = PID(kp=1, ki=0.0, kd=0, integral_limit=2.0)
        ...
        self.log_target_altitude_rate = []
        self.log_actual_altitude_rate = []
        self.log_actual_lateral_rate = []
    ...
    def attitude_loop(self):
        ...
        while rclpy.ok():
            ...
            if self.prev_altitude is not None:
                actual_altitude_rate = (altitude - self.prev_altitude) / dt
            else:
                actual_altitude_rate = 0.0
            self.prev_altitude = altitude

            # Simple proportional desired rate
            target_altitude_rate = 0.5  # Gain = 0.8 m/s per meter error
            altitude_rate_error = target_altitude_rate - actual_altitude_rate
            pitch_cmd = self.altitude_rate_pid.update(altitude_rate_error, dt)
            pitch_cmd = max(min(pitch_cmd, 500), -500)
            pitch_cmd = round(pitch_cmd)

            # --- Lateral Rate Control ---
            if self.prev_lateral is not None:
                actual_lateral_rate = (lateral_error - self.prev_lateral) / dt
            else:
                actual_lateral_rate = 0.0
            self.prev_lateral = lateral_error

            target_lateral_rate = 1.0 * (-lateral_error)  # Oppose error direction
            lateral_rate_error = target_lateral_rate - actual_lateral_rate
            roll_cmd = self.lateral_rate_pid.update(lateral_rate_error, dt)
            ...
            if math.isfinite(altitude):
                ...
                self.log_target_altitude_rate.append(target_altitude_rate)
                self.log_actual_altitude_rate.append(actual_altitude_rate)
                self.log_actual_lateral_rate.append(actual_lateral_rate)
            ...
            if log_counter % 10 == 0:
                self.get_logger().info(
                    ...
                    f"Lateral Offset: {lateral_error:.2f}m | RollCmd: {math.degrees(roll_cmd):.2f}° | Target Altitude Rate: {target_altitude_rate:.2f} | "
                    f"Actual Altitude Rate: {actual_altitude_rate:.2f} | Target Lateral Rate: {target_lateral_rate:.2f} | "
                    f"Actual Lateral Rate: {actual_lateral_rate:.2f}" 
                )
            log_counter += 1
            rate.sleep()
...
def main(args=None):
    ...
    try:
        ...
        with open(csv_filename, mode='w', newline='') as file:
            ...
            for t, offset, roll, altitude_rate, lateral_rate in zip(gps_follower.log_time, gps_follower.log_lateral_offset, gps_follower.log_roll_cmd, gps_follower.log_actual_altitude_rate, gps_follower.log_actual_lateral_rate): #Define what goes in CSV
                writer.writerow([t, offset, roll, altitude_rate, lateral_rate])
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(gps_follower.log_time, gps_follower.log_target_altitude_rate, label="Target Altitude Rate (m/s)")
        plt.plot(gps_follower.log_time, gps_follower.log_actual_altitude_rate, label="Actual Altitude Rate (m/s)", linestyle='--')
        plt.xlabel("Time (s)")
        plt.ylabel("Altitude Rate (m/s)")
        plt.title("Target vs Actual Altitude Rate Over Time")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.show()
